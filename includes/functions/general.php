...
// TABLES: products
  function osc_get_products_name($product_id, $language = '') {
    ...
    $product_query = osc_db_query("select products_name from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language . "'");
...
// Return table heading with sorting capabilities
  function osc_create_sort_heading($sortby, $colnum, $heading) {
    ...
    if ($sortby) {
      $sort_prefix = '<a href="' . osc_href_link(null, osc_get_all_get_params(array('page', 'sort')) . 'page=1&sort=' . $colnum . ($sortby == $colnum . 'a' ? 'd' : 'a')) . '" title="' . osc_output_string(TEXT_SORT_PRODUCTS . ($sortby == $colnum . 'd' || substr($sortby, 0, 1) != $colnum ? TEXT_ASCENDINGLY : TEXT_DESCENDINGLY) . TEXT_BY . $heading) . '" class="productListing-heading">' ;
...
// Get the number of times a word/character is present in a string
  function osc_word_count($string, $needle) {
...
// Return a random value
  ...
  function osc_setcookie($name, $value = '', $expire = 0, $path = '/', $domain = '', $secure = 0) {
    setcookie($name, $value, $expire, $path, (osc_not_null($domain) ? $domain : ''), $secure);
  }

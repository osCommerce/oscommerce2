...
// class methods
    function update_status() {
      ...
      if (!defined('MODULE_PAYMENT_MONEYBOOKERS_STATUS')) {
        ...
        $check_flag = false;
        $check_query = osc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MONEYBOOKERS_SLO_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = osc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_MONEYBOOKERS_SLO_STATUS'");
    ...
    function install() {
      ...
      $zone_query = osc_db_query("select geo_zone_id from " . TABLE_GEO_ZONES . " where geo_zone_name = 'Moneybookers Solo'");
      if (osc_db_num_rows($zone_query)) {
        $zone = osc_db_fetch_array($zone_query);

        $zone_id = $zone['geo_zone_id'];
      } else {
        osc_db_query("insert into " . TABLE_GEO_ZONES . " values (null, 'Moneybookers Solo', 'The zone for the Moneybookers Solo payment module', null, now())");
        $zone_id = osc_db_insert_id();

        $country_query = osc_db_query("select countries_id from " . TABLE_COUNTRIES . " where countries_iso_code_2 = 'GB'");

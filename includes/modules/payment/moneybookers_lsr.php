...
// class methods
    function update_status() {
      ...
      if (!defined('MODULE_PAYMENT_MONEYBOOKERS_STATUS')) {
        ...
        $check_flag = false;
        $check_query = osc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MONEYBOOKERS_LSR_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = osc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_MONEYBOOKERS_LSR_STATUS'");
    ...
    function install() {
      ...
      $zone_query = osc_db_query("select geo_zone_id from " . TABLE_GEO_ZONES . " where geo_zone_name = 'Moneybookers Laser'");
      if (osc_db_num_rows($zone_query)) {
        $zone = osc_db_fetch_array($zone_query);

        $zone_id = $zone['geo_zone_id'];
      } else {
        osc_db_query("insert into " . TABLE_GEO_ZONES . " values (null, 'Moneybookers Laser', 'The zone for the Moneybookers Laser payment module', null, now())");
        $zone_id = osc_db_insert_id();

        $country_query = osc_db_query("select countries_id from " . TABLE_COUNTRIES . " where countries_iso_code_2 = 'IE'");
      ...
      osc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Preparing Order Status', 'MODULE_PAYMENT_MONEYBOOKERS_LSR_PREPARE_ORDER_STATUS_ID', '" . MODULE_PAYMENT_MONEYBOOKERS_PREPARE_ORDER_STATUS_ID . "', 'Set the status of prepared orders made with this payment module to this value', '6', '0', 'osc_cfg_pull_down_order_statuses(', 'osc_get_order_status_name', now())");

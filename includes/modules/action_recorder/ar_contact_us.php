...
*/
  ...
  class ar_contact_us {
    ...
    function canPerform($user_id, $user_name) {
      $check_query = osc_db_query("select date_added from " . TABLE_ACTION_RECORDER . " where module = '" . osc_db_input($this->code) . "' and (" . (!empty($user_id) ? "user_id = '" . (int)$user_id . "' or " : "") . " identifier = '" . osc_db_input($this->identifier) . "') and date_added >= date_sub(now(), interval " . (int)$this->minutes  . " minute) and success = 1 order by date_added desc limit 1");

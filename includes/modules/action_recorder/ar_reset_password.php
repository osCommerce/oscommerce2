...
*/
  ...
  class ar_reset_password {
    ...
    function canPerform($user_id, $user_name) {
      $check_query = osc_db_query("select id from " . TABLE_ACTION_RECORDER . " where module = '" . osc_db_input($this->code) . "' and user_name = '" . osc_db_input($user_name) . "' and date_added >= date_sub(now(), interval " . (int)$this->minutes  . " minute) and success = 1 order by date_added desc limit " . (int)$this->attempts);

...
*/
  ...
  if ($valid_product == false) {
    tep_redirect(tep_href_link('products', 'id=' . (int)$_GET['products_id']));
  }
  ...
  if (isset($_GET['action']) && ($_GET['action'] == 'process') && isset($_POST['formid']) && ($_POST['formid'] == $_SESSION['sessiontoken'])) {
    ...
    if ($error == false) {
      ...
      $email_body .= sprintf(TEXT_EMAIL_LINK, tep_href_link('products', 'id=' . (int)$_GET['products_id'], 'NONSSL', false)) . "\n\n" .
...
<?php echo tep_draw_form('email_friend', tep_href_link(FILENAME_TELL_A_FRIEND, 'action=process&products_id=' . (int)$_GET['products_id']), 'post', '', true); ?>
...
<div class="contentContainer">
  ...
  <div class="buttonSet">
    ...
    <?php echo tep_draw_button(IMAGE_BUTTON_BACK, 'triangle-1-w', tep_href_link('products', 'id=' . (int)$_GET['products_id'])); ?>
  </div>
</div>

...
<?php
  $Qorders = $OSCOM_Db->prepare('select SQL_CALC_FOUND_ROWS o.orders_id, o.date_purchased, o.delivery_name, o.billing_name, ot.text as order_total, s.orders_status_name from :table_orders o, :table_orders_total ot, :table_orders_status s where o.customers_id = :customers_id and o.orders_id = ot.orders_id and ot.class = "ot_total" and o.orders_status = s.orders_status_id and s.language_id = :language_id and s.public_flag = "1" order by o.orders_id desc limit :batch_pageset, :batch_max_results');
  $Qorders->bindInt(':customers_id', $_SESSION['customer_id']);
  $Qorders->bindInt(':language_id', $_SESSION['languages_id']);
  $Qorders->bindInt(':batch_pageset', 1);
  $Qorders->bindInt(':batch_max_results', MAX_DISPLAY_ORDER_HISTORY);
  $Qorders->execute();

  $QordersTotalRows = $OSCOM_Db->query('select found_rows()')->fetchColumn();

  if ($QordersTotalRows > 0) {
...
?>
  ...
  <div class="row">
    <div class="col-sm-6 pagenumber hidden-xs">
      <?php echo HTML::getBatchTotalPages(TEXT_DISPLAY_NUMBER_OF_ORDERS, $QordersTotalRows); ?>
    </div>
    <div class="col-sm-6">
      <div class="pull-right pagenav"><?php echo HTML::getPageLinks($QordersTotalRows, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></div>

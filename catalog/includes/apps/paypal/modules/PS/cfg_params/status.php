  ...
  class OSCOM_PayPal_PS_Cfg_status {
    ...
    function getSetField() {
      $input = '<input type="radio" id="statusSelectionLive" name="status" value="1"' . (OSCOM_APP_PAYPAL_PS_STATUS == '1' ? ' checked="checked"' : '') . '><label for="statusSelectionLive">Live</label>' .
               '<input type="radio" id="statusSelectionSandbox" name="status" value="0"' . (OSCOM_APP_PAYPAL_PS_STATUS == '0' ? ' checked="checked"' : '') . '><label for="statusSelectionSandbox">Sandbox</label>' .
               '<input type="radio" id="statusSelectionDisabled" name="status" value="-1"' . (OSCOM_APP_PAYPAL_PS_STATUS == '-1' ? ' checked="checked"' : '') . '><label for="statusSelectionDisabled">Disabled</label>';

 ...
 */
  ...
  class app_products_action_reviews {
    public static function execute(app $app) {
      ...
      $reviews_breadcrumb_link = tep_href_link('products', 'reviews');
      ...
      if ( !empty($_GET['reviews']) && isset($_GET['id']) && !empty($_GET['id']) ) {
        ...
        if ($new_price = tep_get_products_special_price($Qreview->valueInt('products_id'))) {
          $products_price = '<del>' . $currencies->display_price($Qreview->value('products_price'), tep_get_tax_rate($Qreview->valueInt('products_tax_class_id'))) . '</del> <span class="productSpecialPrice">' . $currencies->display_price($new_price, tep_get_tax_rate($Qreview->valueInt('products_tax_class_id'))) . '</span>';
        } else {
          $products_price = $currencies->display_price($Qreview->value('products_price'), tep_get_tax_rate($Qreview->valueInt('products_tax_class_id')));
        }
        ...
        if ( $Qp->fetch() !== false ) {
          if ($new_price = tep_get_products_special_price($Qp->valueInt('products_id'))) {
            $products_price = '<del>' . $currencies->display_price($Qp->value('products_price'), tep_get_tax_rate($Qp->valueInt('products_tax_class_id'))) . '</del> <span class="productSpecialPrice">' . $currencies->display_price($new_price, tep_get_tax_rate($Qp->valueInt('products_tax_class_id'))) . '</span>';
          } else {
            $products_price = $currencies->display_price($Qp->value('products_price'), tep_get_tax_rate($Qp->valueInt('products_tax_class_id')));
          }

          $products_name = $Qp->value('products_name');

          if (tep_not_null($Qp->value('products_model'))) {
            $products_name .= '<br /><span class="smallText">[' . $Qp->value('products_model') . ']</span>';
          }

          $app->setContentFile('reviews_product.php');

          $reviews_breadcrumb_link = tep_href_link('products', 'reviews&id=' . $_GET['id']);
        }
      }

  ...
  function tep_display_banner($action, $identifier) {
    ...
    if (isset($banner)) {
      if (!empty($banner['banners_html_text'])) {
        ...
        $output = '<a href="' . OSCOM::link('redirect.php', 'action=banner&goto=' . $banner['banners_id']) . '" target="_blank">' . tep_image(DIR_WS_IMAGES . $banner['banners_image'], $banner['banners_title']) . '</a>';
      }

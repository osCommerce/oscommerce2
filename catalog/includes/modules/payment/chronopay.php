    ...
    function install() {
      ...
      if (tep_db_num_rows($check_query) < 1) {
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          tep_db_query("insert into " . TABLE_ORDERS_STATUS . " (orders_status_id, language_id, orders_status_name, public_flag, downloads_flag) values ('" . $status_id . "', '" . $languages[$i]['id'] . "', 'Preparing [ChronoPay]', 0, 0)");
        }
      } else {
        $check = tep_db_fetch_array($check_query);

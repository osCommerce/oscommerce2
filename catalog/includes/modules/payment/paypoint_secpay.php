...
// Incase this gets 'fixed' at the SECPay end do a search and replace on the trans_id too
      ...
      $process_button_string = tep_draw_hidden_field('merchant', MODULE_PAYMENT_PAYPOINT_SECPAY_MERCHANT_ID) .
                               ...
                               tep_draw_hidden_field('callback', tep_href_link('checkout', 'process', 'SSL', false) . ';' . tep_href_link('checkout', 'payment&payment_error=' . $this->code, 'SSL', false)) .
    ...
    function before_process() {
      if ( ($_GET['valid'] == 'true') && ($_GET['code'] == 'A') && !empty($_GET['auth_code']) && empty($_GET['resp_code']) && !empty($_GET[session_name()]) ) {
        ...
        if ($_GET['hash'] != md5($REQUEST_URI . $DIGEST_PASSWORD)) {
          tep_redirect(tep_href_link('checkout', 'payment&' . session_name() . '=' . $_GET[session_name()] . '&payment_error=' . $this->code ."&detail=hash", 'SSL', false, false));
        }
      } else {
        tep_redirect(tep_href_link('checkout', 'payment&' . session_name() . '=' . $_GET[session_name()] . '&payment_error=' . $this->code, 'SSL', false, false));
      }
    }

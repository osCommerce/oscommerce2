...
*/
  ...
  class ht_manufacturer_meta {
    ...
    function execute() {
      ...
      if (basename($PHP_SELF) == FILENAME_DEFAULT) {
        if (isset($HTTP_GET_VARS['manufacturers_id']) && is_numeric($HTTP_GET_VARS['manufacturers_id'])) {
          $meta_info_query = tep_db_query("select mi.manufacturers_seo_description, mi.manufacturers_seo_keywords from " . TABLE_MANUFACTURERS_INFO . " mi where mi.manufacturers_id = '" . (int)$HTTP_GET_VARS['manufacturers_id']  . "' and mi.languages_id = '" . (int)$languages_id . "'");
          ...
          if (tep_not_null($meta_info['manufacturers_seo_keywords'])) {
            $oscTemplate->addBlock('<meta name="keywords" content="' . tep_output_string($meta_info['manufacturers_seo_keywords']) . '" />', $this->group);
          }
        }
      }
    }
    ...
    function keys() {
      return array('MODULE_HEADER_TAGS_MANUFACTURERS_META_STATUS', 'MODULE_HEADER_TAGS_MANUFACTURERS_META_SORT_ORDER');
    }
  }
?>

...
*/
  ...
  class ht_category_meta {
      ...
      if (basename($PHP_SELF) == FILENAME_DEFAULT) {
        if ($current_category_id > 0) {
          ...
          if (tep_not_null($meta_info['categories_seo_description'])) {
            $oscTemplate->addBlock('<meta name="description" content="' . tep_output_string($meta_info['categories_seo_description']) . '" />', $this->group);
          }

          if (tep_not_null($meta_info['categories_seo_keywords'])) {
            $oscTemplate->addBlock('<meta name="keywords" content="' . tep_output_string($meta_info['categories_seo_keywords']) . '" />', $this->group);
          }
        }
      }
    }
    ...
    function keys() {
      return array('MODULE_HEADER_TAGS_CATEGORY_META_STATUS', 'MODULE_HEADER_TAGS_CATEGORY_META_SORT_ORDER');
    }
  }
?>

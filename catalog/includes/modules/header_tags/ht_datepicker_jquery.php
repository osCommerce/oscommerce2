...
*/
  ...
  class ht_datepicker_jquery {
    ...
    function execute() {
      ...
      if (osc_not_null(MODULE_HEADER_TAGS_DATEPICKER_JQUERY_PAGES)) {
        $pages_array = array();
    ...
    function install() {
      ...
      osc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_HEADER_TAGS_DATEPICKER_JQUERY_SORT_ORDER', '900', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
  ...
  function ht_datepicker_jquery_edit_pages($values, $key) {
    ...
    foreach ($files_array as $file) {
      $output .= osc_draw_checkbox_field('ht_datepicker_jquery_file[]', $file, in_array($file, $values_array)) . '&nbsp;' . osc_output_string($file) . '<br />';
    }

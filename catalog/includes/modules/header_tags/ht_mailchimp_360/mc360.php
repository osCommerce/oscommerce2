        ...
        if (!$this->key_valid){
            ...
            if ($api->errorMessage!=''){
                ...
                if (empty($this->store_id)){
                    ...
                    tep_db_query("update " . TABLE_CONFIGURATION . " set configuration_value = '" . tep_db_input($this->store_id) . "' where configuration_key = 'MODULE_HEADER_TAGS_MAILCHIMP_360_STORE_ID'");
                }
            }
        }
    ...
    function process() {
        ...
        foreach($products_array as $product){
            ...
            //now recurse up the categories tree...
                $cat_qry = tep_db_query("select c.categories_id, c.parent_id, cd.categories_name from  " . TABLE_CATEGORIES . " c inner join " . TABLE_CATEGORIES_DESCRIPTION . " cd on c.categories_id = cd.categories_id where c.categories_id =".(int)$cat_id);
                $cats = tep_db_fetch_array($cat_qry);
                if ($cat_name == ''){

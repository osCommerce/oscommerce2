...
// display random review box
        ...
        $reviews_box_contents .= '<div class="text-center"><a href="' . osc_href_link(FILENAME_PRODUCT_REVIEWS, 'products_id=' . $random_product['products_id']) . '">' . osc_image(DIR_WS_IMAGES . $random_product['products_image'], $random_product['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></div><div><a href="' . osc_href_link(FILENAME_PRODUCT_REVIEWS, 'products_id=' . $random_product['products_id']) . '">' . $rand_review_text . '</a>...</div><div class="text-center" title="' .  sprintf(MODULE_BOXES_REVIEWS_BOX_TEXT_OF_5_STARS, $random_product['reviews_rating']) . '">' . osc_draw_stars((int)$random_product['reviews_rating']) . '</div>';
      } elseif (isset($_GET['products_id'])) {
// display 'write a review' box
        $reviews_box_contents .= '<span class="glyphicon glyphicon-thumbs-up"></span> <a href="' . osc_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, 'products_id=' . $_GET['products_id']) . '">' . MODULE_BOXES_REVIEWS_BOX_WRITE_REVIEW .'</a>';
      } else {
// display 'no reviews' box
      ...
      $data = '<div class="panel panel-default">' .
              '  <div class="panel-heading"><a href="' . osc_href_link(FILENAME_REVIEWS) . '">' . MODULE_BOXES_REVIEWS_BOX_TITLE . '</a></div>' .

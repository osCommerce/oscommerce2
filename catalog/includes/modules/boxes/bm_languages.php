      ...
      if (substr(basename($PHP_SELF), 0, 8) != 'checkout') {
        ...
        if (count($lng->catalog_languages) > 1) {
          ...
          foreach($lng->catalog_languages as $key => $value) {
            $languages_string .= ' <a href="' . osc_href_link($PHP_SELF, osc_get_all_get_params(array('language', 'currency')) . 'language=' . $key, $request_type) . '">' . osc_image(DIR_WS_LANGUAGES .  $value['directory'] . '/images/' . $value['image'], $value['name']) . '</a> ';
          }

      ...
      if (substr(basename($PHP_SELF), 0, 8) != 'checkout') {
        ...
        if (count($lng->catalog_languages) > 1) {
          ...
          foreach($lng->catalog_languages as $key => $value) {
            $languages_string .= ' <a href="' . OSCOM::link($PHP_SELF, tep_get_all_get_params(array('language', 'currency')) . 'language=' . $key, 'AUTO') . '">' . HTML::image('includes/languages/' . $value['directory'] . '/images/' . $value['image'], $value['name'], NULL, NULL, NULL, false) . '</a> ';
          }

  ...
  class bm_search {
    ...
    function execute() {
      ...
      $data = '<div class="panel panel-default">' .
              ...
              '    ' . osc_draw_form('quick_find', osc_href_link(FILENAME_ADVANCED_SEARCH_RESULT, '', $request_type, false), 'get') .
              '    <div class="input-group">' .
              '    ' . osc_draw_input_field('keywords', '', 'required aria-required="true" placeholder="' . TEXT_SEARCH_PLACEHOLDER . '"') .
              '      <span class="input-group-btn"><button type="submit" class="btn btn-search"><i class="glyphicon glyphicon-search"></i></button></span>' .
              '    </div>' . osc_draw_hidden_field('search_in_description', '0') . osc_hide_session_id() . '<br />' . MODULE_BOXES_SEARCH_BOX_TEXT . '<br /><a href="' . osc_href_link(FILENAME_ADVANCED_SEARCH) . '"><strong>' . MODULE_BOXES_SEARCH_BOX_ADVANCED_SEARCH . '</strong></a>' .
              '    </form>' .
              '  </div>' .
              '</div>';

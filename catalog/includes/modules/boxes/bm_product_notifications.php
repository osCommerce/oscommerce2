    ...
    function execute() {
      ...
      if ( ($OSCOM_APP->getCode() == 'products') && is_null($OSCOM_APP->getCurrentAction()) && isset($_GET['id']) && !empty($_GET['id']) ) {
        ...
        if ($notification_exists == true) {
          $notif_contents = '<div class="ui-widget-content infoBoxContents"><div><div class="span4"><a href="' . osc_href_link(null, osc_get_all_get_params(array('action')) . 'action=notify_remove', $request_type) . '">' . osc_image(DIR_WS_IMAGES . 'box_products_notifications_remove.gif', IMAGE_BUTTON_REMOVE_NOTIFICATIONS) . '</a></div><div><a href="' . osc_href_link(null, osc_get_all_get_params(array('action')) . 'action=notify_remove', $request_type) . '">' . sprintf(MODULE_BOXES_PRODUCT_NOTIFICATIONS_BOX_NOTIFY_REMOVE, osc_get_products_name($_GET['id'])) .'</a></div></div></div>';
        } else {
          $notif_contents = '<div class="ui-widget-content infoBoxContents"><div><div class="span4"><a href="' . osc_href_link(null, osc_get_all_get_params(array('action')) . 'action=notify', $request_type) . '">' . osc_image(DIR_WS_IMAGES . 'box_products_notifications.gif', IMAGE_BUTTON_NOTIFICATIONS) . '</a></div><div><a href="' . osc_href_link(null, osc_get_all_get_params(array('action')) . 'action=notify', $request_type) . '">' . sprintf(MODULE_BOXES_PRODUCT_NOTIFICATIONS_BOX_NOTIFY, osc_get_products_name($_GET['id'])) .'</a></div></div></div>';
        }

...
*/
  ...
  class ar_admin_login {
    ...
    function canPerform($user_id, $user_name) {
      $check_query = osc_db_query("select id from " . TABLE_ACTION_RECORDER . " where module = '" . osc_db_input($this->code) . "' and (" . (!empty($user_name) ? "user_name = '" . osc_db_input($user_name) . "' or " : "") . " identifier = '" . osc_db_input($this->identifier) . "') and date_added >= date_sub(now(), interval " . (int)$this->minutes  . " minute) and success = 0 order by date_added desc limit " . (int)$this->attempts);
      if (osc_db_num_rows($check_query) == $this->attempts) {
        return false;
      } else {
        return true;
      }
    ...
    function expireEntries() {
      ...
      osc_db_query("delete from " . TABLE_ACTION_RECORDER . " where module = '" . $this->code . "' and date_added < date_sub(now(), interval " . (int)$this->minutes  . " minute)");

      return osc_db_affected_rows($db_link);
    }
    ...
    function install() {
      osc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('Allowed Minutes', 'MODULE_ACTION_RECORDER_ADMIN_LOGIN_MINUTES', '5', 'Number of minutes to allow login attempts to occur.', '6', '0', now())");

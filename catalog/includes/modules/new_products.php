...
*/
  ...
  if ($num_new_products > 0) {
    ...
    while ($new_products = tep_db_fetch_array($new_products_query)) {
      ...
      $new_prods_content .= '<td width="33%" align="center" valign="top"><a href="' . tep_href_link('products', 'id=' . $new_products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $new_products['products_image'], $new_products['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br /><a href="' . tep_href_link('products', 'id=' . $new_products['products_id']) . '">' . $new_products['products_name'] . '</a><br />' . $currencies->display_price($new_products['products_price'], tep_get_tax_rate($new_products['products_tax_class_id'])) . '</td>';

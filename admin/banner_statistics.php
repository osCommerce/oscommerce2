...
// check if the graphs directory exists
  ...
  $banner_query = osc_db_query("select banners_title from " . TABLE_BANNERS . " where banners_id = '" . (int)$_GET['bID'] . "'");
  $banner = osc_db_fetch_array($banner_query);

  $years_array = array();
  $years_query = osc_db_query("select distinct year(banners_history_date) as banner_year from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . (int)$_GET['bID'] . "'");
  while ($years = osc_db_fetch_array($years_query)) {
